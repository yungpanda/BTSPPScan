...
public abstract class AbstractService extends Service {
	...
	static final boolean D = false;			//debug indicator
    ...
    private class IncomingHandler extends Handler { // Handler of incoming messages from clients.
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_REGISTER_CLIENT:
            	if (D) Log.i("MyService", "Client registered: "+msg.replyTo);
                mClients.add(msg.replyTo);
                onServiceStarted();			
                break;
            case MSG_UNREGISTER_CLIENT:
            	if (D) Log.i("MyService", "Client un-registered: "+msg.replyTo);
    ...
    public void onCreate() {
        ...
        if (D) Log.i("MyService", "Service Started.");
    }
    
    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
    	if (D) Log.i("MyService", "Received start id " + startId + ": " + intent);
    ...
    public void onDestroy() {
        ...
        if (D) Log.i("MyService", "Service Stopped.");
    }    
    
    protected void send(Message msg) {
   	 for (int i=mClients.size()-1; i>=0; i--) {
            try {
            	if (D) Log.i("MyService", "Sending message to clients: "+msg);
               mClients.get(i).send(msg);
            }
            catch (RemoteException e) {
                // The client is dead. Remove it from the list; we are going through the list from back to front so this is safe to do inside the loop.
            	if (D) Log.e("MyService", "Client is dead. Removing from list: "+i);
            	mClients.remove(i);
            }
        }    	
   }
